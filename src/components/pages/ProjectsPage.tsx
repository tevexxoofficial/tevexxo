    ...
    <div className="bg-slate-50">
        ...
        <div className="container relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm text-slate-300">
              Every program ends with portfolio-grade projects that mirror what teams ship in
              production.
            </p>
          </div>
        </div>
      ...
      <section className="container mt-8 pb-20">
        {query.isPending ? (
          ...
          <div className={`grid gap-5 ${projects.length > 1 ? "lg:grid-cols-3" : ""}`}>
                  ...
                  {project.technologies.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {project.technologies.map((tech) => (
                        <span
                          key={tech}
                          className="rounded bg-orange-50 px-2 py-1 text-[10px] font-semibold text-orange-600"
                        >
                          {tech}
                        </span>
                      ))}
